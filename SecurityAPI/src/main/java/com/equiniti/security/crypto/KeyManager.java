...
package com.equiniti.security.crypto;
...
import com.equiniti.exception.api.faultcode.SecurityFaultCodes;
...
public class KeyManager {
     ...
     */
    public void validateKey(final String key) throws SecurityException {
        // Validate the key checksum
        if (!validateKeyChecksum(key)) {
        	throw new SecurityException(SecurityFaultCodes.INVALID_KEY_ERROR, null);
        }
        ...
        if (this.blacklist != null) {
            for (String black : this.blacklist) {
                if (key.startsWith(black)) {
                	throw new SecurityException(SecurityFaultCodes.BLACK_LISTED_KEY_ERROR, null);
                }
            }
        }
        ...
        try {
            ...
            throw new SecurityException(SecurityFaultCodes.INVALID_KEY_ERROR, e);
        }
